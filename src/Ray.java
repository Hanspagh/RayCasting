...
public class Ray {
  ...
  protected void cast(Step origin, double range) {
    Step stepX = step(sin, cos, origin.x, origin.y,origin.z, false);
    Step stepY = step(cos, sin, origin.y, origin.x,origin.z, true);
  ...
  protected Step step(double rise, double run, double x, double y,double z, boolean inverted) {
    if (run == 0) return new Step(0,0, 0, 0, 0, Double.POSITIVE_INFINITY, 0, 0);
    double dx = run > 0 ? Math.floor(x + 1) - x : Math.ceil(x - 1) - x;
    double dy = dx * (rise / run);
    return new Step(inverted ? y + dy : x + dx, inverted ? x + dx : y + dy,z, 0, 0, dx * dx + dy * dy, 0, 0);
  }
  ...
  protected Step inspect(Step step, double shiftX, double shiftY, double distance, double offset) {
    ...
    step.height = map.get(step.x - dx, step.y - dy) - step.z;
